...
export class RequestDialog implements OnInit {
  ...
  public courier_reqlist: boolean = false;
  public assis_reqlist: boolean = false;
  ...
  constructor(public reqdialogRef: MatDialogRef<RequestDialog>,
    ...
    this._http.get(api_url, { headers: new HttpHeaders({ 'Authorization': this._userService.getToken() }) })
      .subscribe(res => {
        if (res['status'] == 'ok') {
          ...
          console.log("Dtaes", req_start_date, req_to_date)
          ...
          if (this.data.type == 'courier') {
            ...
            async.forEach(lists, function (list) {
              ...
              console.log("list.from_date", trip_from_date, trip_to_date);
              ...
              if (root.courier_request_lists.length > 0) {
                root.courier_reqlist = true;
              }
              else {
                root.courier_reqlist = false;
              }
            ...
            async.forEach(lists, function (list) {
              ...
              let start_suffixval = root._globalService.ordinal_suffix_of(parseInt(start_date));
              let end_suffixval = root._globalService.ordinal_suffix_of(parseInt(end_date));
              if (list.package_status == 'created') {
              ...
              if (root.assistance_request_lists.length > 0) {
                root.assis_reqlist = true;
              }
              else {
                root.assis_reqlist = false;
              }
...
export class CourierRequestDialog {
  ...
  ngOnInit() {
    this.courier_form.setValue({
      from_date: new Date(this.data.trip.from_date),
      to_date: new Date(this.data.trip.to_date),
  ...
  // public onSelectItemFile(event,i) {
  //   console.log("i",i,"event",event);
  //   let inputEl: HTMLInputElement = this.el.nativeElement.querySelector('#itemphoto_'+i);
  ...
  //           this.item_image_files+"_"+i.push(res['filename']);
  //           // this.image_urls.push(this.api_host + '/static/item_images/' + res['filename']);
  ...
  public onSubmitCourier() {
    console.log("onSubmitCourier", this.courier_form.value);
    if (this.courier_form.valid) {
      let formData = this.courier_form.value;
      if (parseInt(formData.package_weight) <= parseInt(this.ternster_balance_weight)) {
